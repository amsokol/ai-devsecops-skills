...
3. When the pin is already a floating channel, open/update a finding to move to
   the newest cleared concrete tag (major/runtime jumps need Issue unlock —
   [`grouping.md`](../../policy/grouping.md) Majors).
...
2. Edit `uses:` to the cleared tag (prefer `vX.Y.Z`; or full commit SHA when
   product policy requires SHA pinning). **Major-line jumps** only after Issue
   unlock ([`grouping.md`](../../policy/grouping.md)).
...
5. Keep action and image changes grouped per [`grouping.md`](../../policy/grouping.md)
   (majors / runtime images: Issue unlock, then separate routine PR).
